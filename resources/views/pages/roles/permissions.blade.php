                        ...
                        <div class="rounded shadow">
                            <form class="p-3" action="" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <h3>Permission of {{ str_replace(Auth::user()->shop_id ?? '' . '#', '', $role->name) }}
                                    </h3>
                                    <hr>
                                    ...
                                    @foreach ($permissionGroups as $group)
                                        ...
                                        <div class="row {{ $group->group_name }} ml-3">
                                            {{-- @foreach ($permissions as $permission)
                                                <div class="col-md-6 @if ($role->hasPermissionTo($permission)) d-none @endif"
                                                    id="unSelectedPermission_{{ $permission->id }}">
                                                    <div class="form-check form-check-inline">
                                                        <input
                                                            onclick="checkPermission('{{ $group->group_name }}', '{{ $permission->id }}', '{{ $permission->name }}')"
                                                            class="form-check-input" type="checkbox"
                                                            id="permissionCheckbox{{ $permission->id }}"
                                                            name="permissions[]" value="{{ $permission->name }}">
                                                        <label class="form-check-label"
                                                            for="permissionCheckbox{{ $permission->id }}">{{ $permission->name }}</label>
                                                    </div>
                                                </div>
                                            @endforeach --}}
                                        </div>
                                    ...
                                    @foreach ($exist_permissions as $checked_permission)
                                        @if ($role->hasPermissionTo($checked_permission))
                                            <div class="form-check form-check-inline ml-4"
                                                id="provided_permission_{{ $checked_permission->id }}">
                                                <input class="form-check-input" checked
                                                    onclick="delete_permission({{ $checked_permission->id }})"
                                                    type="checkbox" id="checked_{{ $checked_permission->id }}"
                                                    value="">
                                                <label class="form-check-label"
                                                    for="checked_{{ $checked_permission->id }}">{{ $checked_permission->name }}</label>
                ...
                success: function(response) {
                    ...
                    if (response['status'] == 'yes') {
                        ...
                        $("#provided_permission_group_" + group_name).append(
                            '<div class="form-check form-check-inline ml-4" id="provided_permission_' +
                            permission_id + '"><input class="form-check-input" id="checked_' +
                            permission_name + '" checked onclick="delete_permission(' + permission_id +
                            ')" type="checkbox"  value=""><label class="form-check-label" for="checked_' +
                            permission_name + '">' + permission_name +
                            '</label></div><br / id="provided_br_' + permission_id + '">');
                    } else {
                        Toastify({
                            text: response['reason'],
